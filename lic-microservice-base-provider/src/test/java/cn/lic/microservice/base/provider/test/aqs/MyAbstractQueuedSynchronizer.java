...
public abstract class MyAbstractQueuedSynchronizer extends AbstractOwnableSynchronizer implements java.io.Serializable {
     ...
     */
    private Node enq(final Node node) { // node 为绑定了当前线程的 新节点
        for (; ; ) {
            Node t = tail;
            if (t == null) { // Must initialize
                // 没有旧尾巴说明 aqs 是空队列
                // new 一个新节点 并用 cas 设置到  aqs 的 head 属性上
                if (compareAndSetHead(new Node())) {
                    // cas 成功后 将 tail 属性指向为 head。
                    tail = head;
                }
                // 继续自旋
            } else {
                // 如果 aqs 有尾巴，则将要入列的节点的前驱节点指向为 aqs 的尾节点
                node.prev = t;
                // 并用 cas 设置 aqs 的尾节点为要入列的节点
                if (compareAndSetTail(t, node)) {
                    // 设置成功的话 将 尾节点的后继节点设置为 当前要入列的节点
                    t.next = node;
                    // 返回原来的尾节点，打破自旋。
                    return t;
                }
    ...
    private Node addWaiter(Node mode) {
        ...
        Node pred = tail;
        if (pred != null) {
            // 如果有尾巴，把当前构造的新节点的上一个节点设置为 旧尾巴
            node.prev = pred;
            // 用 cas 设置aqs的尾节点为当前构造的新节点
            if (compareAndSetTail(pred, node)) {
                // 设置 旧尾巴的下一个节点为 当前构造的新节点
                pred.next = node;
                // 返回 当前构造的新节点
                return node;
            }
        }
        // 如果aqs队列没有尾节点  或者 aqs有尾巴但是用cas设置aqs新尾巴为构造的node的时候失败的话
        // 自旋将构造的新节点入列
        enq(node);
        // 返回当前构造的新节点
        return node;
    ...
    private void cancelAcquire(Node node) {
        // Ignore if node doesn't exist //跳过空节点
        if (node == null) {
            return;
        }
        ...
        // Skip cancelled predecessors // 跳过 cancelled 的前驱节点，直到它不是（waitStatus > 0 是 cancelled 节点)
        Node pred = node.prev;
        ...
        Node predNext = pred.next;  // 记录过滤后的节点的后置节点

        // Can use unconditional write instead of CAS here.
        // After this atomic step, other Nodes can skip past us.
        // Before, we are free of interference from other threads.
        node.waitStatus = Node.CANCELLED;  // 将当前节点状态改为CANCELLED

        // 如果当前节点是tail尾节点 则将从后往前找到那个非取消状态的节点设为tail尾节点
        // If we are the tail, remove ourselves.
        if (node == tail) {
            if (compareAndSetTail(node, pred)) {
                // 用 cas 设置那个找到的非取消状态的节点的下一个节点为 空
            ...
            if (
                /// 如果当前节点不是首节点的后置节点
                    pred != head
                            //并且
                            //如果前置节点的状态是SIGNAL 或者 状态小于0并且设置状态为SIGNAL成功
    ...
    private static boolean shouldParkAfterFailedAcquire(Node pred, Node node) {
        // 这个方法一般在 自旋操作中调用， 判断竞锁失败后是否需要 挂起
        ...
        if (ws > 0) {
            /**
             * 前驱节点的 waitStatus 如果是 cancelled ，跳过它继续重试
             */
            /*
             * Predecessor was cancelled. Skip over predecessors and
             * indicate retry.
             */
            do {  // 从后往前依此跳过cancelled状态的节点, 直到 waitStatus <= 0 即不是 cancelled 状态
                node.prev = pred = pred.prev;
                /**
                 * 上面相当于：
                 * pred = pred.prev;
            ...
            // 将找到的符合标准的节点的后置节点指向当前节点，node 节点的前驱节点已经在 do 中设置为了 pred
            pred.next = node;
        } else {  // 这里的 waitStatus <= 0 且 != -1 （-1已经在上面返回true了）
            /*
             ...
             */
            /**
             * waitStatus 一定 是 0 或者 PROPAGATE，以此表明我们需要 SIGNAL 状态，但是先别急着挂起。
             * 调用者需要重试来确保挂起之前不能竞锁到。
             */
            // cas 设置 waitStatus 为 -1 状态
    ...
    final boolean acquireQueued(final Node node, int arg) {
        ...
        try {
            ...
            for (; ; ) {
                ...
                // 如果节点是头节点 则尝试获取锁
                if (p == head) {
                    if (tryAcquire(arg)) {  // tryAcquire 不会操作 node
                        // 如果锁获取成功 则将... 并返回不需挂起的标识
                        setHead(node); // 设置当前 node 为 head，置空 thread 和 prev，方便 退出队列
                        p.next = null; // help GC  ，原 head 节点的 next 置空。
                        failed = false; // 未发生错误
                        return interrupted;
                    }
                    // 竞锁失败继续自旋
                ...
                /**
                 *  说明p不是头节点  或者  p是头节点但是获取锁失败
                 */
                // 判断竞锁失败后是否需要挂起，
                /**
                 * 在 shouldParkAfterFailedAcquire 方法中：
                 * 如果 p 节点为signal，则返回 true，
                 * 如果 p 节点为cancelled，则节点前溯且跳过状态为cancelled 的节点，找到非cancelled 的节点后停止，并将
                 *        p 设置为找到的节点，返回 false
                 * 如果 p 节点为 CONDITION 或 PROPAGATE 则使用 cas 设置 p节点的 waitStatus 为 signal， 返回 false
                 */
                if (shouldParkAfterFailedAcquire(p, node)) {
                    // 挂起线程并返回检查中断的结果
                    if (parkAndCheckInterrupt()) {
                        // 在 tryAcquire 失败并确定线程挂起之后，将 interrupted 设为 true
                        interrupted = true;
                    }
                }
                // 继续自旋 竞锁
    ...
    public final void acquire(int arg) {
        ...
        /**
         * tryAcquire 失败后 才会进行 addWaiter 和 acquireQueued，否则不执行
         *
         * !tryAcquire(arg);  // 抢锁是否失败
         * addWaiter(Node.EXCLUSIVE); //生成 node
         * acquireQueued(addWaiter(Node.EXCLUSIVE), arg); // 独占入列是否成功
         */
        if (!tryAcquire(arg)) {
            // 创建 waiterNode，用 cas 排队到 aqs 的尾节点，cas 失败那就继续 自旋 cas 设置，直到排进
            Node waiterNode = addWaiter(Node.EXCLUSIVE);
            // 自旋竞锁，根据情况决定是否要中断挂起
            if (acquireQueued(waiterNode, arg)) {
                selfInterrupt();
            }
        }
    }
    ...
    public final boolean hasQueuedPredecessors() {
        ...
        // 2) : 头节点的下一个节点为空或者头节点的下一个节点绑定的线程就是当前线程
        // 满足上面条件则代表 AQS 有
        return h != t && ((s = h.next) == null || s.thread != Thread.currentThread());
        // 返回false代表队列中没有节点或者仅有一个节点是当前线程创建的节点。返回true则代表队列中存在等待节点，
        // 当前线程需要入队等待
    }
     ...
     */
    private static final Unsafe unsafe = getUnsafe(); //Unsafe.getUnsafe();
    ...
    static {
        try {
            // 这 3 个 offset 是绑在  AbstractQueuedSynchronizer class 上的
            stateOffset = unsafe.objectFieldOffset(java.util.concurrent.locks.AbstractQueuedSynchronizer.class.getDeclaredField("state"));
            headOffset = unsafe.objectFieldOffset(java.util.concurrent.locks.AbstractQueuedSynchronizer.class.getDeclaredField("head"));
            tailOffset = unsafe.objectFieldOffset(java.util.concurrent.locks.AbstractQueuedSynchronizer.class.getDeclaredField("tail"));
            // 注意 下两个属性是绑在 Node  class 上的
